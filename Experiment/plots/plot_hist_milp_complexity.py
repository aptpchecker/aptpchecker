...
def extract_neuralsat_results(args, names):
    ...
    for name in names:
        ...
        # total
        if len(parts) == 5:
            complexities = extract_milp_complexity(res_file)
            runtime = float(parts[-1])
            if parts[2] == 'certified':
                if runtime <= TIMEOUT:
                    results['solved'].extend(complexities)
                else:
                    results['unsolved'].extend(complexities)
                    # print(res_file)
            else:
...
if __name__ == '__main__':
    ...
    plt.xlim(-50, 3000)
    plt.tight_layout()
    plt.title(NAMES[args.verifier])
    
    plt.xlabel('MILP Complexity')
    plt.ylabel('Counts')
    
    folder = os.path.join(FIGURE_DIR, args.verifier)
    os.makedirs(folder, exist_ok=True)
    
    filename = f'{folder}/MILP_COMPLEXITY_{args.postfix}.{args.format}'
