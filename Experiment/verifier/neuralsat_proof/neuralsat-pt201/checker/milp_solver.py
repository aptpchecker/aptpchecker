...
def _build_solver_relu(model, layer, layer_name, prev_vars):
    ...
    print(f'[MILP] Added: constrained={count_a_vars}, free={len(vars_array.reshape(-1))-count_a_vars}, total={len(vars_array.reshape(-1))} variables')
...
def _build_solver_conv2d(model, layer, layer_name, layer_bounds, prev_vars, refine):
    ...
    this_layer_shape = (1,) + layer_bounds[0].shape
    assert len(prev_layer_shape) == len(this_layer_shape) == 4
    # print(f'{prev_layer_shape=} {this_layer_shape=}')
    # this layer weight
    weight = layer.weight.clone().detach().numpy()
    bias = layer.bias.clone().detach().numpy()
    # print(f'Layer {weight.shape=} {bias.shape=} {np.array(prev_vars).shape=} {layer_bounds[0].shape=}')
    ...
    for out_chan_idx in range(this_layer_shape[1]):
        ...
        for out_row_idx in range(this_layer_shape[2]):
            ...
            for out_col_idx in range(this_layer_shape[3]):
                ...
                var = model.addVar(
                    lb=layer_bounds[0][out_chan_idx, out_row_idx, out_col_idx].item(), 
                    ub=layer_bounds[1][out_chan_idx, out_row_idx, out_col_idx].item(), 
                    obj=0, 
                    vtype=grb.GRB.CONTINUOUS, 
                    name=f'lay{layer_name}_{neuron_idx}'
                )
    ...
    assert np.array(layer_vars).shape == layer_bounds[0].shape
    model.update()
...
def build_milp_solver(net, input_lower, input_upper, timeout=15.0, c=None, name='', refine=True):
    # abstract domain
    ...
    for layer_name, layer in enumerate(layers):
        ...
        if isinstance(layer,  nn.Linear):
            ...
            new_layer_gurobi_vars = _build_solver_conv2d(
                model=solver, 
                layer=layer,
                layer_name=layer_name,
                layer_bounds=hidden_bounds[layer_name],
...
if __name__ == "__main__":
    from .abstractor import SimpleConvModel2
    net = SimpleConvModel2()
    
    lower = torch.randn(1, 28, 28)
    upper = lower + 0.01
    
    solver = build_milp_solver(net, lower, upper, timeout=1.0, refine=False)[0]
    print(solver)