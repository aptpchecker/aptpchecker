...
def _proof_worker_impl(candidate):
    ...
    #     can_model.optimize()
    print(f'Solved leaf: {can_node = } in {time.time() - start_solve_time} seconds')
...
class ProofChecker:
    ...
    def build_core_checker(self, objectives, timeout=15.0, refine=False):
    ...
    def prove(self, proofs, batch=1, expand_factor=2.0, timeout=3600.0, timeout_per_proof=3600.0, timeout_per_neuron=15.0, refine=False):
        print(f'{expand_factor=} {refine=}')
        ...
        core_solver_model = self.build_core_checker(self.dnf_objectives, timeout=timeout_per_neuron, refine=refine)
        ...
        while len(proof_objectives):
            ...
            proof_tree = proofs.get(current_id, [])
            print(f"Processing objective {current_id}: {len(proof_tree)=} {expand_factor=}")
            if not len(proof_tree):
                continue
            current_proof_queue = ProofQueue(proofs=proof_tree)
            
            # step 2.3: update shared model
            MULTIPROCESS_MODEL = shared_solver_model
            
            # step 2.4: prove nodes
            print(f'{proofs.get(current_id)=}')
            print(f'{current_objective.cs=}')
            ...
            while len(current_proof_queue):
                ...
                for solved_node in results:
                    ...
                    # else:
                    #     return ProofReturnStatus.TIMEOUT # unproved
