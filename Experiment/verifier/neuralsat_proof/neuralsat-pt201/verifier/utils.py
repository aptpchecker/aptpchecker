...
def _preprocess(self: verifier.verifier.Verifier, objectives: typing.Any, force_split: str | None = None) -> tuple:
    ...
    if len(objectives) and (Settings.use_mip_tightening) and self.abstractor.method == 'backward':
        ...
        if Settings.disable_refine_conv:
...
def _setup_restart(self: verifier.verifier.Verifier, nth_restart: int, objective: typing.Any) -> None | dict:
    ...
    if (not self.input_split) and Settings.use_restart and self.num_restart == len(HIDDEN_SPLIT_RESTART_STRATEGIES) and Settings.use_mip_tightening:
        if 'forward' in abstract_method:
            ...
            pass
        elif Settings.disable_refine_conv and any([isinstance(_, (torch.nn.Conv2d, torch.nn.Conv3d, torch.nn.ConvTranspose2d, torch.nn.ConvTranspose3d)) for _ in self.net.modules()][1:]):
...
def _to_json(proof_tree, objective):
    json_obj = copy.deepcopy(objective)
    json_obj['proof_nodes'] = []
    
    print(proof_tree)
    for node in proof_tree:
        json_obj['proof_nodes'].append((node, 'relu'))
...
@beartype
def get_proof_tree(self: verifier.verifier.Verifier, dnf_objectives: typing.Any) -> None | dict:
    unsat_core = self.get_unsat_core()
    if not unsat_core:
        return None
    
    # print(f'{self.domains_list.var_mapping=}')
    # print(f'{self.domains_list.layer_mapping=}')
    # exit()
    # obj_dict = {}
    # while len(dnf_objectives):
    #     objective = dnf_objectives.pop(1)
    #     obj_dict[objective.ids.item()] = {
    #         'output_lhs': objective.cs[0].numpy().tolist(), 
    #         'output_rhs': objective.rhs[0].numpy().tolist(), 
    #         'input_lowers': objective.lower_bounds[0].numpy().tolist(),
    #         'input_uppers': objective.upper_bounds[0].numpy().tolist(),
    #     }
    ...
    for obj_idx, conflict_clauses in unsat_core.items():
        ...
        # _to_json(proof_tree[obj_idx], obj_dict[obj_idx])
