...
class LinearTransformer(nn.Module):
    ...
    def __init__(self, layer):
        ...
        self.weight = layer.weight.to(DTYPE)
        self.bias = layer.bias.to(DTYPE)
...
class ReLUTransformer(nn.Module):
    ...
    def forward(self, center, error):
        ...
        new_error = torch.zeros((n_error + n_new_error, d), dtype=DTYPE)
        new_error[:n_error] = error
        new_error[:, ub[0] <= 0] = 0

        # new center
        new_center = torch.zeros(center.size(), dtype=DTYPE)
