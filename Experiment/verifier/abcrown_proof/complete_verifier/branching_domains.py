...
class BatchedDomainList(AbstractDomainList):
    ...
    def add(self, bounds, d, check_infeasibility):
        ...
        indexer = torch.all(bounds['lower_bounds'][self.final_name] <= decision_threshs[:batch], dim=1)
        
        if infeasible is not None:
            indexer = torch.logical_and(indexer, torch.logical_not(infeasible))
            
        indexer = indexer.nonzero().view(-1)
        conflict_batch_indexer_lst = [ci for ci in range(batch) if ci not in indexer]
        # print(f'{indexer=}')
        # print(f'{conflict_batch_indexer_lst=}')
        ...
        if len(conflict_batch_indexer_lst) > 0:
            ...
            # print(f'######\n{self.histories=}\n######\n')
            # print(f'{len(conflict_batch_indexer_lst)=} {self.conflict_histories=}')
