...
# proof checker
from objective import Objective, DnfObjectives
    ...
    def complete_verifier(
        ...
        for property_idx, properties in enumerate(batched_vnnlib):  # loop of x
            ...
            if enable_incomplete and not input_split:
                ...
                l, nodes, ret, conflict_nodes = self.bab(
                ...
                magic_number = 3
                self.proofs[property_idx + magic_number] = conflict_nodes
            ...
            # record objectives here
            proof_inputs = torch.stack([data_min.flatten(), data_max.flatten()], dim=-1)
            print(f'{proof_inputs.shape=}')
            print(f'{c.shape=}')
            print(f'{rhs.shape=}')
            objectives.append(Objective((
                proof_inputs.detach().cpu().numpy().tolist(), 
                (c[0].detach().cpu().numpy(), rhs[0].detach().cpu().numpy())
            )))
            
        else:
            
            dnf_objectives = DnfObjectives(objectives, vnnlib_shape, False)
             
            checker = ProofChecker(
                net=model_ori,
                input_shape=vnnlib_shape,
                dnf_objectives=copy.deepcopy(dnf_objectives),
                verbose=False,
            )
            prove_timeout = timeout_threshold - (time.time() - start_time)
            print(f'{prove_timeout=}')
            ...
            prove_status = checker.prove(
                ...
                timeout_per_neuron=20.0,
                timeout_per_proof=prove_timeout,
                timeout=prove_timeout,
                expand_factor=2.0, 
                refine=True,
            )
            prove_time = time.time()-prove_tic
            print(f'{prove_status=}, {prove_time=}')
            
            return f'safe,{prove_status},{prove_time}'
    ...
    def main(self, interm_bounds=None):
        ...
        for new_idx, csv_item in enumerate(example_idx_list):
            ...
            if (not verified_success
                ...
                # if verified_status.startswith('safe'):
                #     print(f'{self.proofs=}')
