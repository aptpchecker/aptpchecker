...
def split_domain(net, domains, d, batch, impl_params=None, stats=None,
    ...
    stats.visited += batch # len(domains) - old_d_len
    domains.print()
    stats.timer.add('add')
...
def history_to_clause(h: dict, name_mapping: dict) -> list:
    clause = []
...
def general_bab(net, domain, x, refined_lower_bounds=None,
    ...
    input_relu_splitter = (InputReluSplitter() if branching_input_and_activation else None)
    ...
    return global_lb, stats.visited, result, conflict_nodes
